...
async function getNewNeighbors(node, debtor, creditor) {
	const neighbors = await BlockchainSplitwise.getCreditors(node);
	if (node !== debtor)
		return neighbors
	return neighbors?.includes(creditor) ? neighbors : [...(neighbors ?? []), creditor];
}
...
async function getTotalOwed(user) {
	const data = await BlockchainSplitwise.users(user);
	return data?.totalOwed ?? 0;
}
...
async function getLastActive(user) {
	const data = await BlockchainSplitwise.users(user);
	const lastActive = data?.lastActive?.toNumber()
...
async function add_IOU(creditor, amount) {
	const loop = await doBFS(creditor, defaultAccount, async (node) => getNewNeighbors(node, defaultAccount, creditor));
	//console.log(loop)
	await BlockchainSplitwise.add_IOU(creditor, amount, loop ? loop : []);
}
